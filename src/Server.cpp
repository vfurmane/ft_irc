...
int	Server::_handle_message(epoll_event &event)
{
	...
	else
	{
		...
		while (peer.hasCompleteMessage())
		{
			...
			try
			{
				if (!peer.isRegistered() && message.command != "PASS" && message.command != "NICK" && message.command != "USER" && message.command != "CAP")
...
void	Server::_handleReadyFds(int event_count, struct epoll_event *events)
{
	for (int i = 0; i < event_count; i++)
	{
		...
		{
#ifndef NDEBUG
			std::cerr << "Received new message from " << this->peers.get(events[i].data.fd).generatePrefix() << std::endl;
#endif
