...
#include "../src/commands/privmsg/nickname.cpp"

TEST_CASE("PRIVMSG - nickname")
{
	...
	{
		command_privmsg_nickname(deps, peer, "nick1", "hello world");
	...
	SECTION("when the nickname is in use")
	{
		...
		command_privmsg_nickname(deps, peer, "nick1", "hello world");
		REQUIRE( std::find(g_send_arg_buf.begin(), g_send_arg_buf.end(), PrivmsgMessage(peer, "nick1", "hello world", true).input + std::string(CRLF)) != g_send_arg_buf.end() );
	};
};
